...
#include "Includes\assimp\assimp\Importer.hpp"
#include "Includes\assimp\assimp\scene.h"
#include "Includes\assimp\assimp\postprocess.h"
...
public:
	...
	Model(const char* path, GLuint program) {
...
private:
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		if (mesh->mMaterialIndex >= 0)
		{
			...
			// 1. Diffuse maps
			vector<TTexture> diffuseMaps = this->loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse");
			textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
			// 2. Specular maps
			vector<TTexture> specularMaps = this->loadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular");
	...
	vector<TTexture> loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
	{
		vector<TTexture> textures;
